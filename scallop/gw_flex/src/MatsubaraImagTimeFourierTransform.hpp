...
//
//
//			bN0_[iw] = 0.041666666666666664 - CD(0,0.453125)*pn - (193*std::pow(pn,2))/384. + CD(0,0.3869357638888889)*std::pow(pn,3)
//						+ (25987*std::pow(pn,4))/107520. - CD(0,0.12823331318204365)*std::pow(pn,5) - (787147*std::pow(pn,6))/1.327104e7
//						+ CD(0,0.0243327226588335)*std::pow(pn,7);
//
//			bN1_[iw] =0.041666666666666664 + CD(0,0.17447916666666666)*pn + std::pow(pn,2)/8. - CD(0,0.0886501736111111)*std::pow(pn,3)
//						- (11089*std::pow(pn,4))/322560. + CD(0,0.014633614676339286)*std::pow(pn,5) + (214993*std::pow(pn,6))/4.644864e7 -
//					   CD(0,0.0014226102744881227)*std::pow(pn,7);
//		}
...
//
//			bN0_[iw] = -(CD(0,-2.0) + pn + 6.0*std::exp(CD(0,2.0*pn))*pn + 4.0*std::exp(CD(0,pn))*(CD(0,-1.0) + pn)
...
//
//			bN1_[iw] = (-12.0*std::exp(CD(0,2.0*pn))*pn + 8.0*std::exp(CD(0,pn))*(CD(0,-1.0) + pn) + 8.0*std::exp(CD(0,3.0*pn))*(CD(0,1.0) + pn)
...
//	buffFNMm2_ = buffFNMm1_ = buffF1_ = buffF0_ = std::vector<T>(nGm*this->get_data_block_size(),T(0));
//}
...
//				buffFNMm2_[ig*nB+ib] = this->read_data_ptr_block(id,static_cast<int>(nM)-2)[ib];
//				buffFNMm1_[ig*nB+ib] = this->read_data_ptr_block(id,static_cast<int>(nM)-1)[ib];
//			}
		}
	}
	...
	if ( timeToFreqTransform )
	{
		for (size_t ig = 0 ; ig < nG ; ++ig )
		{
			...
			for (size_t iw = 0 ; iw < nM ; ++iw )
			{
				...
				//We set the unbalanced frequency to zero
				if ( (not isFermi_) and (nM % 2 == 0) and ( frequencyIndex == -static_cast<int>(nM/2) ) )
				{
					for ( size_t ib = 0; ib < nB; ++ib)
						ptr_block[ib] = 0;
					continue;
